...
## Load libraries, load data, clean data, set color palettes
...
setwd("~/Carp/Figures")
pdf(file="Figure_1B.pdf", width= 7.086, height =6)
...
legend('topleft', legend=levels(sizeCut), lwd=scaled, col='black', xpd=TRUE, inset=c(-.1, -.1), cex=.8)
dev.off()
...
setwd("~/Carp/Figures")
pdf(file="Figure_1C.pdf", width= 7.086, height =6)
C
dev.off()
```
### Figure 2. Spatiotemporal data streams for Site 1
```{r}
...
setwd("~/Carp/Figures")
pdf(file="Figure_2.pdf", width= 7.086, height =7.086)
...
### Figure 3. Feeding bout group size, duration, and correlation
```{r feeding_bouts}
...
setwd("~/Carp/Figures")
pdf(file="Figure_3.pdf", width= 7.086, height =7.086)
plot_grid(top_row, C, labels=c("", "     (C)"), ncol = 1, label_fontface="plain")
dev.off()  
...
### Figure 4. Superfeeders (Unique Daily Detections)
...
### Figure 4.v2 Superfeeders (Total Daily Detections)
This is the version that appears in the manuscript.
...
setwd("~/Carp/Figures")
pdf(file="Figure_4.pdf", width= 7.086, height =7.086)
...
### Figure 5. Co-feeding network heterogeneity and connectivity
...
setwd("~/Carp/Figures")
pdf(file="Figure_5.pdf", width= 10, height =4)
...
### Figure S5. Superfeeder vs. length (2020)
```{r}
...
```
### Lorenz Curves and Gini Coefficients for 2019 Seasons
```{r}
...
### Appendix Figure 7. Superfeeder vs. non-superfeeder date of first detection
```{r}
#2020
...
```
### Compare superfeeders in 2019 vs. 2020
```{r compare-superfeeders}
top20_ID_2019
length(top20_ID_2019)
top20_ID_2020
length(top20_ID_2020)
