...
summarize(n_distinct (PIT)) -> kens1
kens1%>% print(n=200)


names(kens1) <- c("species" ,"date", "count")
head(kens1)
as.Date(kens1$date, "%m/%d/%Y", tx - "CST") -> kens1$date	

subset(kens1, date <= "0018-10-08") -> kens1

		
kens1 <- droplevels(kens1)

xyplot(count ~ date,kens1, group=species, xlab="Date",ylab="Daily Unique Count", main="Species", factor=1, jitter.x=TRUE,key = list(text = list(as.character(unique(kens1$species))), points = list(pch = 20:23, col = 1:3)), cex=2,pch = 20,21,1,23, col = 1:3)



xyplot((log10(count)+1) ~ date,kens1, group=species, xlab="Date",ylab="LOG Daily Unique Count", main="Species",factor=1, jitter.x=TRUE,key = list(text = list(as.character(unique(kens1$species))), points = list(pch = 20:23, col = 20:23)), pch = 20:23, col = 20:23)
